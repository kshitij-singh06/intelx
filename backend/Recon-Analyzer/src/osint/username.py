...
class Sagemode:
    def __init__(self, username: str, found_only=True):
    ...
    def is_soft404(self, html_response: str) -> bool:
        """Check if a page is a soft 404 (looks like success but is actually not found)."""
    ...
    def start(self):
        """Start searching for the username across all sites."""
        logger.info(f"Starting username search for: {self.username}")
        ...
        try:
            ...
            for thread in threads:
                thread.join(timeout=15)  # Timeout per thread

            logger.info(f"Username search complete. Found on {self.positive_count} sites.")
